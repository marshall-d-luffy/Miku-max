...
module.exports = {
    ...
    onStart: async function ({ message, event, args }) {
        ...
        if (mention.length == 0 &&  !event?.messageReply) return message.reply("Please mention someone");
        else if (mention.length == 1 || event.messageReply) {
            const one = event.senderID, two = mention[0] || event.messageReply.senderID;
